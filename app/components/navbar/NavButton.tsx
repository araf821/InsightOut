...
const NavButton: FC<NavButtonProps> = ({ icon: Icon, title, onClick }) => {
  return (
    <button
      onClick={onClick}
      className="group relative flex w-full items-center justify-center gap-2 overflow-hidden bg-primary p-3 text-lg text-zinc-800 outline-none transition-all duration-200 hover:scale-[1.01] hover:outline-primary focus:ring-2 ring-offset-2 focus:ring-primary md:text-xl lg:gap-4 lg:text-2xl"
    >
      {Icon && <Icon />}
